...
export function injectImageTags(source: string, images: ImageInsertion[]): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    // 标准形态:tag 部分保持裸文本(与存量格式一致),nl 部分包 <nl> 子标签
    for (const image of inserted) {
      const inner = image.nl ? `${image.tag}<nl>${image.nl}</nl>` : image.tag;
      output += `${insertedEol}<bbi_image>${inner}</bbi_image>`;
    }
    output += line.eol;
  }
  return output;
