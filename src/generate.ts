 ...
 *
 * 口径:显式指定 > 面板固定种子(>0)> 随机。
 * NAI/Latent 与 ComfyUI 的随机上界不同(各自协议的合法范围),故分开取。
 */
export function decideSeed(backend: BackendId, override?: number): number {
  ...
  if (backend === 'latent') return settings.latent.seed > 0 ? settings.latent.seed : randomSeed();
  return randomSeed();
}
...
export async function generateImage(
  ...
  // Latent 渠道:完全复用 NAI 机器(latentAsNai 映射后走 generateNaiImage),
  // 负面 = 渠道级(映射自渠道 negativePrompt)+ 本画面 <negative>,逗号合并;
  // 有意不占 NAI 官方 API 的并发闸门——闸门护的是官方 NAI 账号,Latent 指向第三方
  // 站点(0.3.0 引入渠道时即如此),限流退避由 generateNaiImage 自带。
  const latentView = status.backend === 'latent' ? latentAsNai(settings.latent) : null;
  const pictureNegative = (input.negative ?? '').trim();
  if (latentView && pictureNegative) {
    latentView.undesiredContent = [latentView.undesiredContent.trim(), pictureNegative]
      .filter(Boolean)
      .join(', ');
  }
  ...
  try {
    // 持槽:取到槽位后本函数内一直持有,直到 finally 释放(Latent 不占官方闸门,见上)
