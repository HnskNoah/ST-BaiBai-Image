...
/** 把设置调成「Latent 渠道已配齐」,渠道级负面清空(走 Anima 默认回落)。 */
function latentReady(): void {
  settings.defaultBackend = 'latent';
  settings.latent.url = 'https://latent.example';
  settings.latent.key = 'k';
  settings.latent.seed = 0;
  settings.latent.negativePrompt = '';
...
describe('latent 渠道', () => {
  it('与 NAI 共用闸门:取槽后才发请求,请求抛错也照放', async () => {
    latentReady();
    const release = vi.fn();
    vi.mocked(acquireNaiSlot).mockResolvedValue(release);
    ...
    // 站点在途超限是 409,与 NAI 的 429 一样靠客户端闸门堵住;漏放槽就永久少一格
    vi.mocked(generateNaiImage).mockRejectedValueOnce(new Error('409'));
    await expect(generateImage({ prompt: '1girl', seed: 1 })).rejects.toThrow('409');
    expect(release).toHaveBeenCalledTimes(2);
  });
