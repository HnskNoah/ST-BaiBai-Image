...
插件把目标正文按原始换行逐行编号（空行也占行号），AI 只返回源码行号和提示词。插件在本地把 tag 插到对应行之后，不让模型回传或改写完整正文。

行号只服务于首次插入。正式 tag 写入后，tag 自身随正文移动；后续图片渲染不再依赖旧行号。

### 2.3 不静默截断上下文

插件不提供“最大上下文字数”或“最大上下文 Token 数”限制。

- 保留“携带最近几层”的数量设置，默认建议为 2 层。
- 被选中的楼层发送完整正文。
- 柏宝书状态快照完整发送。
...
- 完整原文。
- 明确标记哪一层是本次目标楼层。
- 仅给目标楼层的每个源码行加 `[L0001]` 形式的行号；此前上下文不加行号。
...
- API 不存在、版本不兼容或读取失败时，退回最近 N 层正文。
...
- `line` 必须是目标正文中存在的整数源码行号。
- 行号表示把 tag 插在该行之后；空行同样有行号，但提示词要求模型优先选择承载画面完成时刻的非空行。
...
如果行号越界、不是整数或 `tag` 为空，整次响应校验失败，不写入正文；`nl` 缺失宽容降级为纯 tag。
...
首次插入多个 tag 时，应先验证所有行号并一次性构造新正文，避免连续保存导致中间状态以及重复触发其他插件。
...
- 监听新 AI 正文完成事件。
- 拼装最近 N 层完整正文和 D1/D2 柏宝书快照。
- 给目标正文加源码行号并要求模型返回 JSON。
- 校验行号、提示词和最大数量后安全写回正文。
